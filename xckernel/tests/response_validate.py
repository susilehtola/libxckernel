...
def _pert_fields(D, chi, dchi):
    """Perturbed fields rho^X, grad rho^X on the grid from a perturbed DM.

    chi: (nao, ng); dchi: (3, nao, ng)."""
...
def check_pyscf(family: str):
    ...
    _, vxc, fxc = dft.libxc.eval_xc(xc, rho0, spin=0, deriv=2)[:3]

    if xctype == "LDA":
        chi = ao.T; dchi = np.zeros((3,) + chi.shape)
        grad_rho = None
        libxc = {"vrho": vxc[0], "v2rho2": fxc[0]}
    else:
        chi = ao[0].T; dchi = np.transpose(ao[1:4], (0, 2, 1))
        grad_rho = rho0[1:4]
        libxc = {"vrho": vxc[0], "vsigma": vxc[1],
                 "v2rho2": fxc[0], "v2rhosigma": fxc[1], "v2sigma2": fxc[2]}

    rho1, grad1 = _pert_fields(dm1, chi, dchi)
    base = {"w": grids.weights, "chi": chi, "dchi": dchi, "grad_rho": grad_rho}
    pert = {"p1": {"rho": rho1, "grad": grad1}}
...
def check_third_order(family: str):
    ...
    name = {"lda": "LDA_X", "gga": "GGA_X_PBE"}[family]
    ...
    rho1, grad1 = fields(D1)
    rho2, grad2 = fields(D2)
    base = {"w": w, "chi": chi, "dchi": dchi, "grad_rho": grad0}
    pert = {"p1": {"rho": rho1, "grad": grad1},
            "p2": {"rho": rho2, "grad": grad2}}
