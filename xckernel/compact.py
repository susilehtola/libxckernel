...
#: direction-resolved density-Hessian rows (spatial-gradient operands)
...
for _s in ("a", "b"):
    ...
    VECTOR_GROUPS[f"dgrad_rho_{_s}_h"] = \
        tuple(f"dgrad_rho_{_s}_h_{ax}" for ax in "xyz")
    VECTOR_GROUPS[f"d2grad_rho_{_s}_gh"] = \
        tuple(f"d2grad_rho_{_s}_gh_{ax}" for ax in "xyz")
for _slot in ("h", "gh"):
